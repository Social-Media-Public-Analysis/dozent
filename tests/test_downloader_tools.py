...
class DownloaderToolsTestCase(unittest.TestCase):
    ...
    def test_make_progress_status(self):
        ...
        for progress in [
            ...
            DownloadProgress(
                dl_size=1024, filesize=1048576, speed=42, status="downloading"
            ),
            DownloadProgress(
                dl_size=129864, filesize=129865, speed=777, status="paused"
            ),
            DownloadProgress(
                dl_size=999999, filesize=999999, speed=999, status="finished"
            ),
        ]:
            ...
            progress_percentage = (
                100.0 * progress.dl_size / progress.filesize if progress.filesize else 0
            )
            ...
            (
                ...
                actual_prefix,
                actual_suffix,
            ) = DownloaderTools._make_progress_status(downloader_obj)
    ...
    def test_download_with_pysmartdl(self):
        ...
        DownloaderTools.download_with_pysmartdl(
            link="http://ipv4.download.thinkbroadband.com/20MB.zip",
